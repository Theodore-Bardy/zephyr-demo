 ...
 * @brief     Agent managing Wi-Fi interface
 */
...
#else
_Static_assert(0 != strcmp(CONFIG_WIFI_SSID, ""),
               "Wi-Fi SSID must not be empty");
_Static_assert(strlen(CONFIG_WIFI_SSID) <= 32,
               "Wi-Fi SSID must be 32 characters or less");
#endif
...
#else
_Static_assert(0 != strcmp(CONFIG_WIFI_PASSWORD, ""),
               "Wi-Fi password must not be empty");
_Static_assert(strlen(CONFIG_WIFI_PASSWORD) >= 8
                   && strlen(CONFIG_WIFI_PASSWORD) <= 64,
               "Wi-Fi password must be between 8 and 64 characters");
#endif
...
bool
wifi_agent_connect (void)
{
    if (WIFI_AGENT_STATE_CONNECTED == current_state)
    {
        LOG_INF("Wi-Fi agent is already connected");
        return true;
    }

    if (WIFI_AGENT_STATE_IDLE != current_state)
    {
...
bool
wifi_agent_is_connected (size_t delay_ms)
{
...
void
wifi_agent_get_mac_address (char *mac_address)
{
...
K_THREAD_DEFINE(wifi_agent_thread_id,
                WIFI_AGENT_THREAD_STACK_SIZE,
                prvWifiAgentThread,
                NULL,
                NULL,
                NULL,
                WIFI_AGENT_THREAD_PRIORITY,
                0,
                0);
